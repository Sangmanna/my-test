...
public class CountryConverter {
	...
	public static String iso3ToIso2CountryCode(String iso) {
		String countryCode = "";
		if (iso != null && !iso.isEmpty()) {
			initCountryCodeMapping();
			countryCode = localeMap.get(iso).getCountry();
		}
		return countryCode;
	}

	public static String iso2ToIso3CountryCode(String iso) {
		String countryCode = "";
		
		if (iso != null && !iso.isEmpty()) {
			initCountryCodeMapping();
			Locale locale = Locale.of("", iso);
	...
	public static String unknownIsoToIso2CountryCode(String iso) {
		if (iso == null) {
			return "";
		}

		return iso.length() == 2 ? iso : iso3ToIso2CountryCode(iso);
	}

	public static String unknownIsoToIso3CountryCode(String iso) {
		if (iso == null) {
			return "";
		}

		return iso.length() == 3 ? iso : iso2ToIso3CountryCode(iso);
	}

    public static String getIso2FromFullName(String country){
    	String countryFound = "";
		
    	if (country != null && !country.isEmpty()) {
			Map<String, String> map = countryCodeMap();
			countryFound = map.get(country);
		}
		
		if ("".equals(countryFound)) {
    ...
    public static String unknownToIso2(String country) {
    	if(country != null && country.length() > 3) {country = getIso2FromFullName(country);}
    	else {country = unknownIsoToIso2CountryCode(country);}    	
    	return country;
    }

    public static String unknownToIso3(String country) {
    	if(country != null && country.length() > 3) {country = getIso2FromFullName(country);}
    	return unknownIsoToIso3CountryCode(country);
    }   
