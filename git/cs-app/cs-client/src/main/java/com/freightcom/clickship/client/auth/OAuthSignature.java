...
import com.freightcom.clickship.client.util.URLUtil;
...
public class OAuthSignature {
    ...
    public static Map<String, String> getAsMap(OAuthConfig config, String endpoint, HttpMethod httpMethod, Map<String, String> params) {
        ...
        if(config.getAccessToken() == null || config.getTokenSecret() == null){
            oAuthSignature = generateOAuthSignature(config.getConsumerSecret(), endpoint, httpMethod, authParams);
        } else {
            authParams.put(OAuthHeader.OAUTH_TOKEN.getValue(), config.getAccessToken());
            oAuthSignature = generateOAuthSignature(config.getConsumerSecret(), config.getTokenSecret(), endpoint, httpMethod, authParams);
        }
    ...
    private static String generateOAuthSignature(String customerSecret, String tokenSecret, String endpoint, HttpMethod httpMethod, Map<String, String> parameters) {
        String signatureBaseString = getSignatureBaseString(endpoint, httpMethod.name(), parameters);
        // v1, v2
        String secret = customerSecret + SpecialSymbol.AMP.getPlain() + tokenSecret;
    ...
    private static String getSignatureBaseString(String url, String method, Map<String, String> parameters) {
        String requestURL = URLUtil.urlEncode(url);
        // 1. Percent encode every key and value that will be signed.
        Map<String, String> encodedParameters = sortAndEncodeParameters(parameters);

//        // 2. Sort the list of parameters alphabetically by encoded key.
//        encodedParameters = SortUtil.getSortedParameters(encodedParameters);
    ...
    private static Map<String, String> sortAndEncodeParameters(Map<String, String> parameters) {
        Map<String, String> encodedParamsMap = new TreeMap<>();
